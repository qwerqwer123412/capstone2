...
def ap_detail(ap_name, n_intervals):
    ...
    for i in window:
        ...
        node_feats = g['AP'].x[idx]
        rxs.append(node_feats[1].item())    # rx_bytes
        txs.append(node_feats[2].item())    # tx_bytes
    ...
    fig = make_subplots(
        ...
        subplot_titles=[f"{ap_name} 예측 vs 실측", "RX/TX/Clients"]
    )
    ...
    fig.add_trace(go.Scatter(x=times, y=clients, name="Clients", mode="lines+markers"), row=2, col=1)
    fig.update_yaxes(title_text="Bytes / Count", row=2, col=1)
    ...
    idx = AP_NAME2IDX[ap_name]
    meta = g_meta['AP'].meta[idx]
    meta_df = pd.DataFrame([
        {'항목':'AP','값':ap_name},
        {'항목':'MAC','값':meta.get('mac','')},
        {'항목':'채널','값':int(g_meta['AP'].x[idx][3].item())},
    ])
    meta_table = dash_table.DataTable(
        columns=[{"name":"항목","id":"항목"},{"name":"값","id":"값"}],
        data=meta_df.to_dict('records'),
        style_header={'display':'none'}, style_cell={'padding':'8px','textAlign':'left'}
    )
